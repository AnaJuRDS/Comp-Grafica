...
function animateClouds() {
  ...
  drawLine(cableLineVertices, [0.0, 0.5, 0.0, 1.0]); // Cabo
  for (const petala of petalas) {
    drawCircle(petala, [0.6, 0.4, 0.8, 1.0]); // Pétalas
  }
  drawCircle(mioloVertices, [1.0, 1.0, 0.0, 1.0]); // Miolo
  
  // Anima as nuvens
  for (let i = 0; i < nuvens.length; i++) {
    const offsetX = Math.sin(cloudOffset + i * 0.3) * 0.08;
    const offsetY = Math.cos(cloudOffset + i * 0.3) * 0.00;
    const cloud = nuvens[i].map((coord, index) => index % 2 === 0 ? coord + offsetX : coord + offsetY);
    drawCircle(cloud, [1.0, 1.0, 1.0, 1.0]); // Nuvens
  }
  
  // Atualiza o deslocamento para a animação
  cloudOffset += 0.01;

  // Chama a animação de novo no próximo quadro
